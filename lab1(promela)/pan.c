...
short src_ln2 [] = {
	  0,  77,  77,  78,  78,  78,  78,  78, 
	 76,  81,  76,  81,  83,  83,  84,  84, 
	 84,  82,  86,  82,  86,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
...
short src_ln1 [] = {
	  0,  57,  57,  58,  58,  60,  61,  62, 
	 62,  63,  63,  63,  61,  65,  61,  65, 
	 65,  59,  67,  67,  56,  70,  56,  53, 
	 71,  53,  71,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "Philosophers.pml", 1, 26 },
	{ "-", 27, 28 }
};
uchar reached1 [] = {
	  0,   1,   1,   1,   0,   1,   0,   1, 
...
short src_ln0 [] = {
	  0,  11,  13,  15,  17,  18,  16,  21, 
	 22,  20,  24,  26,  27,  25,  30,  31, 
	 29,  13,  35,  35,  36,  40,  41,  39, 
	 44,  45,  43,  10,  49,  10,  49,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "Philosophers.pml", 1, 30 },
	{ "-", 31, 32 }
};
uchar reached0 [] = {
	  0,   1,   0,   1,   1,   0,   0,   1, 
...
{	int j, h = now._nr_pr;
	...
	case 1:	/* checker */
		...
		((P1 *)pptr(h))->p = 0;
#ifdef VAR_RANGES
		logval("checker:i", ((P1 *)pptr(h))->i);
		logval("checker:p", ((P1 *)pptr(h))->p);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* phil */
		...
		((P0 *)pptr(h))->i = par0;
		/* locals: */
		((P0 *)pptr(h))->disposable = 0;
#ifdef VAR_RANGES
		logval("phil:disposable", ((P0 *)pptr(h))->disposable);
		logval("phil:i", ((P0 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("forklocations[l_in]", now.forklocations[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("hasEaten[l_in]", now.hasEaten[l_in]);
		}
	}
#endif
}
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			printf("	int    forklocations[%d]:	%d\n", l_in, now.forklocations[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			printf("	int    hasEaten[%d]:	%d\n", l_in, now.hasEaten[l_in]);
		}
	}
}
void
...
{	/* int i; */
	...
	printf("	int    i:	%d\n", ((P2 *)pptr(pid))->i);
		break;
	case 1:
		printf("local vars proc %d (checker):\n", pid);
	printf("	int    i:	%d\n", ((P1 *)pptr(pid))->i);
	printf("	int    p:	%d\n", ((P1 *)pptr(pid))->p);
		break;
	case 0:
	...
	printf("	int    i:	%d\n", ((P0 *)pptr(pid))->i);
	printf("	byte   disposable:	%d\n", ((P0 *)pptr(pid))->disposable);
		break;
	}
}
void
